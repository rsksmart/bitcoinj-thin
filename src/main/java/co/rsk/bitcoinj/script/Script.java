...
public class Script {
    ...
    public TransactionWitness createEmptyWitness(BtcECKey key) {
    ...
    public int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) throws IllegalStateException {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
    ...
    private static void executeCheckLockTimeVerify(BtcTransaction txContainingThis, int index, LinkedList<byte[]> stack, Set<VerifyFlag> verifyFlags) throws ScriptException {
        if (stack.size() < 1)
    ...
    private static void executeCheckSequenceVerify(BtcTransaction txContainingThis, int index, LinkedList<byte[]> stack, Set<VerifyFlag> verifyFlags) throws ScriptException {
        if (stack.size() < 1)
    ...
    private static void checkSequence(long nSequence, BtcTransaction txContainingThis, int index) {
...
/*        } catch (SignatureDecodeException e) {
     ...
     */
    public void correctlySpends(BtcTransaction txContainingThis, int scriptSigIndex, @Nullable TransactionWitness witness, @Nullable Coin value,
        ...
        if (ScriptPattern.isP2WPKH(scriptPubKey)) {
            ...
            BtcECKey pubkey = BtcECKey.fromPublicOnly(witness.getPush(1));
            Script scriptCode = new ScriptBuilder().data(ScriptBuilder.createP2PKHOutputScript(pubkey.getPubKey()).getProgram())
                .build();
