...
public class RedeemScriptParserFactory {
    ...
    public static RedeemScriptParser get(List<ScriptChunk> chunks) {
        ...
        if (scriptParserResult.getChunks().equals(chunks)) {
            logger.debug("[get] Received redeem script matches the testnet federation hardcoded one. Return NonStandardRedeemScriptParser");
            return new NonStandardRedeemScriptParser();
        }

        if (chunks.size() < 4) {
            // A multisig redeem script must have at least 4 chunks (OP_N [PUB1 ...] OP_N CHECK_MULTISIG)
            logger.trace("[get] Less than 4 chunks, return NonStandardRedeemScriptParser");
            return new NonStandardRedeemScriptParser();
        }
        ...
        logger.debug("[get] Return NonStandardRedeemScriptParser");
        return new NonStandardRedeemScriptParser();
    }
