...
public class RedeemScriptParserFactory {
    ...
    public static RedeemScriptParser get(List<ScriptChunk> chunks) {
        ...
        Script redeemScript = scriptBuilder.addChunks(chunks).build();
        ...
        boolean isFlyover = hasFastBridgePrefix(chunks);
        if (isFlyover) {
            List<ScriptChunk> chunksWithoutFlyover = chunks.subList(2, chunks.size());
            scriptBuilder = new ScriptBuilder();
            Script redeemScriptWithoutFlyover = scriptBuilder.addChunks(chunksWithoutFlyover).build();

            if (redeemScriptWithoutFlyover.isSentToMultiSig()) {
                logger.debug("[get] Return FastBridgeRedeemScriptParser");
                return new FastBridgeRedeemScriptParser(
                    chunks
                );
            }
            if (hasP2shErpRedeemScriptStructure(redeemScriptWithoutFlyover.getChunks())) {
                logger.debug("[get] Return FastBridgeP2shErpRedeemScriptParser");
                return new FastBridgeP2shErpRedeemScriptParser(
                    chunks
                );
            }
            if (hasLegacyErpRedeemScriptStructure(redeemScriptWithoutFlyover.getChunks())) {
        ...
        if (hasP2shErpRedeemScriptStructure(chunks)) {
