...
package co.rsk.bitcoinj.deprecated.crypto;
...
public final class HDKeyDerivation {
    ...
    public static DeterministicKey createMasterPubKeyFromBytes(byte[] pubKeyBytes, byte[] chainCode) {
        return new DeterministicKey(ImmutableList.<ChildNumber>of(), chainCode, new LazyECPoint(BtcECKey.CURVE.getCurve(), pubKeyBytes), null, null);
    }
    ...
    public static DeterministicKey deriveChildKey(DeterministicKey parent, ChildNumber childNumber) throws HDDerivationException {
        if (!parent.hasPrivKey()) {
            ...
            return new DeterministicKey(
                    ...
                    rawKey.chainCode,
                    new LazyECPoint(BtcECKey.CURVE.getCurve(), rawKey.keyBytes),
                    null,
                    parent);
        } else {
    ...
    public static RawKeyBytes deriveChildKeyBytesFromPrivate(DeterministicKey parent,
        ...
        BigInteger ki = priv.add(ilInt).mod(BtcECKey.CURVE.getN());
    ...
    public static RawKeyBytes deriveChildKeyBytesFromPublic(DeterministicKey parent, ChildNumber childNumber, PublicDeriveMode mode) throws HDDerivationException {
        ...
        final BigInteger N = BtcECKey.CURVE.getN();
        ECPoint Ki;
        switch (mode) {
            case NORMAL:
                Ki = BtcECKey.publicPointFromPrivate(ilInt).add(parent.getPubKeyPoint());
                break;
            case WITH_INVERSION:
                ...
                Ki = BtcECKey.publicPointFromPrivate(ilInt.add(RAND_INT).mod(N));
                BigInteger additiveInverse = RAND_INT.negate().mod(N);
                Ki = Ki.add(BtcECKey.publicPointFromPrivate(additiveInverse));
    ...
    private static void assertNonInfinity(ECPoint point, String errorMessage) {
        if (point.equals(BtcECKey.CURVE.getCurve().getInfinity()))
            throw new HDDerivationException(errorMessage);
    }

    private static void assertLessThanN(BigInteger integer, String errorMessage) {
        if (integer.compareTo(BtcECKey.CURVE.getN()) > 0)
            throw new HDDerivationException(errorMessage);
    }
