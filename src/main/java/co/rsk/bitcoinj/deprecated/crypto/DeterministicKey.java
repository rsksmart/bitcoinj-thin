...
import co.rsk.bitcoinj.deprecated.core.Base58;
 ...
 */
public class DeterministicKey extends BtcECKey {

    /** Sorts deterministic keys in the order of their child number. That's <i>usually</i> the order used to derive them. */
    public static final Comparator<BtcECKey> CHILDNUM_ORDER = new Comparator<BtcECKey>() {
        @Override
    ...
    public DeterministicKey(ImmutableList<ChildNumber> childNumberPath,
                            ...
                            BigInteger priv,
                            @Nullable DeterministicKey parent) {
        super(priv, compressPoint(BtcECKey.publicPointFromPrivate(priv)));
        checkArgument(chainCode.length == 32);
        this.parent = parent;
        this.childNumberPath = checkNotNull(childNumberPath);
        this.chainCode = Arrays.copyOf(chainCode, chainCode.length);
    ...
    public static DeterministicKey deserialize(NetworkParameters params, byte[] serializedKey, @Nullable DeterministicKey parent) {
        ...
        if (pub) {
            return new DeterministicKey(path, chainCode, new LazyECPoint(BtcECKey.CURVE.getCurve(), data), parent, depth, parentFingerprint);
        } else {
     ...
     * in which case the time is stored alongside the key as per normal, see {@link BtcECKey#getCreationTimeSeconds()}.
     */
    @Override
